...
type AssertFailure =
    {
        ...
        Actual: string
        /// <summary>Structured metadata describing the observed value or exception.</summary>
        ActualObservedInfo: TestifyObservedInfo
        /// <summary>Structured metadata describing the expected observation when Testify can recover it.</summary>
...
module Assert =
    ...
    /// <remarks>
    /// <para>
    /// The quotation is evaluated by this call. Use <c>result</c> when you want to inspect the result
    ...
    /// Fail-fast twin that raises immediately instead of returning an <c>AssertResult</c>.
    /// </seealso>
    ...
    /// </code>
    /// </example>
    val result: expectation: AssertExpectation<'T> -> actual: Expr<'T> -> AssertResult

    /// <summary>
    /// Evaluates an assertion like <c>result</c>, but uses the supplied test label in failure output.
    /// </summary>
    ...
    /// <seealso cref="M:Testify.Assert.result``1(Testify.AssertExpectation{``0},Microsoft.FSharp.Quotations.FSharpExpr{``0})">
    /// Use <c>result</c> when the rendered quotation is already descriptive enough.
    /// </seealso>
    /// <example id="assert-result-named-1">
    /// <code lang="fsharp">
    /// let result =
    ///     Assert.resultNamed
    ...
    /// </code>
    /// </example>
    val resultNamed: test: string -> expectation: AssertExpectation<'T> -> actual: Expr<'T> -> AssertResult
    ...
    /// <seealso cref="M:Testify.Assert.shouldAsync``1(Testify.AssertExpectation{``0},Microsoft.FSharp.Quotations.FSharpExpr)">
    /// Use <c>shouldAsync</c> when the surrounding test should fail immediately instead of receiving a result.
    /// </seealso>
    /// <example id="assert-result-async-1">
    /// <code lang="fsharp">
    /// let! result =
    ///     Assert.resultAsync
    ...
    /// </code>
    /// </example>
    val resultAsync: expectation: AssertExpectation<'T> -> actual: Expr -> Task<AssertResult>
    ...
    /// </exception>
    /// <seealso cref="M:Testify.Assert.result``1(Testify.AssertExpectation{``0},Microsoft.FSharp.Quotations.FSharpExpr{``0})">
    /// Use <c>result</c> when you want to inspect the structured outcome before deciding whether to raise.
    /// </seealso>
    ...
    /// <remarks>
    /// <para>
    /// Collector helpers store non-throwing result values directly. Results are stored in
    ...
    module Collect =
        ...
        /// <summary>Stores an assertion result and returns it.</summary>
        /// <param name="collector">The collector that should receive the result.</param>
        /// <param name="result">The assertion result to store.</param>
    ...
    /// <seealso cref="M:Testify.Assert.result``1(Testify.AssertExpectation{``0},Microsoft.FSharp.Quotations.FSharpExpr{``0})">
    /// Non-throwing twin that returns an <c>AssertResult</c> for later inspection or rendering.
    /// </seealso>
    ...
    /// </exception>
    /// <seealso cref="M:Testify.Assert.resultAsync``1(Testify.AssertExpectation{``0},Microsoft.FSharp.Quotations.FSharpExpr)">
    /// Async non-throwing twin that returns the structured assertion result inside a task.
    /// </seealso>
    /// <example id="assert-should-async-1">
    /// <code lang="fsharp">
    /// do!
    ///     Assert.shouldAsync
    ///         (AssertExpectation.equalTo 3)
