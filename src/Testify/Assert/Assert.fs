...
module Assert =
    ...
    let private toFailure
        ...
        : AssertFailure =
        let actualObservedInfo = Observed.toInfo observed

        let expectedObservedInfo =
            let display =
                expectation.Details observed
                |> Option.bind _.ExpectedValue
                |> Option.orElseWith (fun () ->
                    if expectation.Label.StartsWith("Throws", System.StringComparison.Ordinal) then
                        Some expectation.Description
                    elif expectation.Label = "DoesNotThrow"
                         || expectation.Label = "DoesNotThrowAsync"
                         || not actualObservedInfo.IsException then
                        Some expectation.Description
                    else
                        None)

            let isExpectedException =
                expectation.Label.StartsWith("Throws", System.StringComparison.Ordinal)

            display
            |> Option.map (fun text ->
                {
                    Display = Some text
                    IsException = isExpectedException
                    ExceptionType = None
...

        {
            ...
            ActualObservedInfo = actualObservedInfo
            ExpectedObservedInfo = expectedObservedInfo
    ...
    /// <summary>
    /// Like <c>result</c>, but uses an explicit human-readable test label in the failure output.
    /// </summary>
    let resultNamed
        (test: string)
        (expectation: AssertExpectation<'T>)
        ...
        | Failed failure ->
...

            {
                TestifyReport.create
                    ...
                    $"Failed test: {failure.Test}" with
                    Test = Some failure.Test
                    Expectation = Some failure.Expected
                    Expected = Some failure.Expected
                    Actual = Some failure.Actual
                    ExpectedObservedInfo = failure.ExpectedObservedInfo
                    ActualObservedInfo = Some failure.ActualObservedInfo
    ...
    let toDisplayStringWith
        ...
        match result with
        | Passed ->
            TestifyReport.renderStatusWith options "assertion" "passed" "Test passed."
        | Failed _ ->
            result
            |> toFailureReport
    ...
    let shouldAsync
        ...
        : Task =
        task {
            let! result = resultAsync expectation actual
            assertPassed result
        }
