...
module AssertExpectation =
    ...
    let private becauseUnexpectedValue
        (expected: 'T)
        (actual: 'T)
        : string =
        $"Tested code returned {Render.formatValue actual} but expected {Render.formatValue expected}."

    let private becauseUnexpectedValueOrFastDiff
        (expected: 'T)
        (actual: 'T)
        : string =
        match Diff.tryDescribe expected actual with
        | Some diff when not (diff.StartsWith("Structural diff:", System.StringComparison.Ordinal)) ->
            diff
        | _ ->
            becauseUnexpectedValue expected actual
        ...
        createValue
            ...
            (fun actual ->
                if actual = expected then
                    ...
                    Some (becauseUnexpectedValueOrFastDiff expected actual))
    ...
    let equalBy
        ...
        (expected: 'T)
        : AssertExpectation<'T> =
        let projectedExpected = projection expected

        createValue
            "EqualBy"
            $"match {Render.formatValue expected} after projection"
            (fun actual -> projection actual = projectedExpected)
            Render.formatValue
            (fun actual ->
                let projected = projection actual

                if projected = projectedExpected then
                    None
                else
                    Some
                        $"Projected value was {Render.formatValue projected} \
                        but projected expected value was \
    ...
    let equalByKey
        ...
        createValue
            "EqualByKey"
            $"have projected value {Render.formatValue expectedKey}"
            (fun actual -> projection actual = expectedKey)
            Render.formatValue
            (fun actual ->
                let projected = projection actual

                if projected = expectedKey then
                    None
                else
                    Some
                        $"Projected value was {Render.formatValue projected} \
