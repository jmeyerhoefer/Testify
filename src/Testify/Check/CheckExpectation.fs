...
module CheckExpectation =
    ...
    let private becauseReturnedUnexpectedValue
        (expected: 'T)
        (actual: 'T)
        : string =
        $"Tested code returned {Render.formatValue actual} but expected {Render.formatValue expected}."
            ...
            (fun _ actual reference ->
                ...
                | Result.Ok actualValue, Result.Ok referenceValue ->
                    ...
                    | true, true ->
                        None
                    | false, true ->
                        Some (becauseReturnedUnexpectedValue expected actualValue)
                    | true, false ->
                        Some
                            $"Reference returned {Render.formatValue referenceValue} \
                            but expected {Render.formatValue expected}."
                    | false, false ->
    ...
    let isTrue<'Args> : CheckExpectation<'Args, bool, bool> =
        equalTo true

    let isFalse<'Args> : CheckExpectation<'Args, bool, bool> =
        equalTo false
    ...
    let equalBy<'Args, 'T, 'Key when 'Key: equality>
        (projection: 'T -> 'Key)
        : CheckExpectation<'Args, 'T, 'T> =
        create
            "EqualBy"
    ...
    let equalByKey<'Args, 'T, 'Key when 'Key: equality>
        (projection: 'T -> 'Key)
        (expectedKey: 'Key)
        : CheckExpectation<'Args, 'T, 'T> =
        create
            "EqualByKey"
            $"produce projected key {Render.formatValue expectedKey} on both sides"
            (fun _ actual expected ->
                match actual, expected with
                | Result.Ok actualValue, Result.Ok expectedValue ->
                    projection actualValue = expectedKey
                    && projection expectedValue = expectedKey
                | Result.Error actualEx, Result.Error expectedEx ->
                    actualEx.GetType () = expectedEx.GetType ()
                | _ ->
                    false)
            ...
            (fun _ actual expected ->
                ...
                | Result.Ok actualValue, Result.Ok expectedValue ->
                    ...
                    match projectedActual = expectedKey, projectedExpected = expectedKey with
                    | true, true ->
                        None
                    | false, true ->
                        Some
                            $"Projected tested value was {Render.formatValue projectedActual} \
                            instead of {Render.formatValue expectedKey}."
                    | true, false ->
                        Some
                            $"Projected reference value was {Render.formatValue projectedExpected} \
                            instead of {Render.formatValue expectedKey}."
                    | false, false ->
                        Some
                            $"Neither side projected to {Render.formatValue expectedKey}. \
                            Tested code projected to {Render.formatValue projectedActual}; \
                            reference projected to {Render.formatValue projectedExpected}."
                | Result.Error actualEx, Result.Error expectedEx
                    when actualEx.GetType () <> expectedEx.GetType () ->
                    Some
                        $"Tested code threw {Render.formatException actualEx} \
                ...
                | Result.Error actualEx, Result.Ok _ ->
                    Some
                        $"Tested code threw {Render.formatException actualEx} \
                        before the projected key could be checked."
                | Result.Ok _, Result.Error expectedEx ->
                    Some
                        $"Reference behavior threw {Render.formatException expectedEx} \
                        before the projected key could be checked."
    ...
    let equalToReferenceBy<'Args, 'T, 'Key when 'Key: equality>
        (projection: 'T -> 'Key)
        : CheckExpectation<'Args, 'T, 'T> =
        equalBy projection
