...
module private CheckCore =
    ...
    let runBy
        ...
        try
            let property = buildProperty verify
            FsCheck.Check.One (resolvedConfig, property)
            finalize expectation state
        with ex ->
            Errored $"Check runner threw {Render.formatException ex}."
...
module Check =
    ...
    let checkBy
        ...
        : CheckResult<'Args, 'Actual, 'Expected> =
        let config = configuredDefaultConfig ()
        CheckCore.runBy config buildProperty actual reference expectation
    ...
    let checkEqualBy<'Args, 'T, 'Key when 'T: equality and 'Key: equality>
        ...
        (actual: Expr<'Args -> 'T>)
        : CheckResult<'Args, 'T, 'T> =
        check (CheckExpectation.equalBy projection) reference actual
    ...
    let checkEqualUsingBy<'Args, 'T, 'Key when 'T: equality and 'Key: equality>
        ...
        (actual: Expr<'Args -> 'T>)
        : CheckResult<'Args, 'T, 'T> =
        checkUsing arbitrary (CheckExpectation.equalBy projection) reference actual
    ...
    let checkEqualUsingComparer
        ...
        (actual: Expr<'Args -> 'T>)
        : CheckResult<'Args, 'T, 'T> =
        check (CheckExpectation.equalWith comparer) reference actual
    ...
    let checkEqualUsingComparerUsing
        ...
        (actual: Expr<'Args -> 'T>)
        : CheckResult<'Args, 'T, 'T> =
        checkUsing arbitrary (CheckExpectation.equalWith comparer) reference actual
    ...
    let checkBeTrueBy
        ...
        (actual: Expr<'Args -> bool>)
        : CheckResult<'Args, bool, bool> =
        checkBy buildProperty (CheckExpectation.equalTo true) (constantReference true) actual

    let checkBeTrueByWith
        (config: FsCheck.Config)
        (buildProperty: (('Args -> bool) -> FsCheck.Property))
        (actual: Expr<'Args -> bool>)
        : CheckResult<'Args, bool, bool> =
        checkByWith config buildProperty (CheckExpectation.equalTo true) (constantReference true) actual
    ...
    let toFailureReport
        ...
        | Exhausted message ->
            ...
            |> TestifyReport.withInferredHints
            |> Some
        | Errored message ->
            {
                TestifyReport.create
        ...
        | Failed failure ->
...

            {
                TestifyReport.create
                    ...
                    $"Failed property case: {failure.Test}" with
                    ...
                    OriginalExpectedObservedInfo =
                        if showOriginal then Some (Observed.toInfo failure.Original.ExpectedObserved)
                        else None
                    OriginalActualObservedInfo =
                        if showOriginal then Some (Observed.toInfo failure.Original.ActualObserved)
                        else None
                    ...
                    ExpectedObservedInfo =
                        Some
                            (
                                failure.Shrunk
                                |> Option.map _.ExpectedObserved
                                |> Option.defaultValue failure.Original.ExpectedObserved
                                |> Observed.toInfo
                            )
                    ActualObservedInfo =
                        Some
                            (
                                failure.Shrunk
                                |> Option.map _.ActualObserved
                                |> Option.defaultValue failure.Original.ActualObserved
                                |> Observed.toInfo
                            )
                    ShrunkExpectedObservedInfo =
                        failure.Shrunk
                        |> Option.map (fun shrunkCase -> Observed.toInfo shrunkCase.ExpectedObserved)
                    ShrunkActualObservedInfo =
                        failure.Shrunk
                        |> Option.map (fun shrunkCase -> Observed.toInfo shrunkCase.ActualObserved)
    ...
    let toDisplayStringWith
        ...
        match result with
        | Passed ->
            TestifyReport.renderStatusWith options "property" "passed" "Property passed."
        | Exhausted _
        | Errored _
        | Failed _ ->
            result
    ...
    let assertPassed
        ...
        | Failed failure ->
            ...
            let reportOptions = TestExecution.currentReportOptions ()

            match toFailureReport result with
            | Some report ->
                TestExecution.recordFailureReport report

                TestifyRunnerContext.dispatchFailure
                    {
                        Report = report
                        Rendered = TestifyReport.renderWith reportOptions report
                        OutputFormat = (TestifyReportOptions.normalize reportOptions).OutputFormat
                    }
            | None ->
                let rendered = toDisplayStringWith reportOptions result

                match (TestifyReportOptions.normalize reportOptions).OutputFormat with
                | OutputFormat.Json -> failwith rendered
    ...
    module Collect =
        ...
        let assertAll
            ...
            if not failures.IsEmpty then
                ...
                let reports =
                    failures
                    |> List.choose toFailureReport

                let rendered =
                    TestifyReport.renderCollectionWith
                        reportOptions
                        "propertyCollection"
    ...
    let shouldBy
        ...
        (actual: Expr<'Args -> 'Actual>)
        : unit =
        checkBy buildProperty expectation reference actual
        |> assertPassed
    ...
    let shouldByWith
        ...
        (actual: Expr<'Args -> 'Actual>)
        : unit =
        checkByWith config buildProperty expectation reference actual
        |> assertPassed
    ...
    let shouldBeTrueBy
        ...
        (actual: Expr<'Args -> bool>)
        : unit =
        checkBeTrueBy buildProperty actual
        |> assertPassed

    let shouldBeTrueByWith
        (config: FsCheck.Config)
        (buildProperty: (('Args -> bool) -> FsCheck.Property))
        (actual: Expr<'Args -> bool>)
        : unit =
        checkBeTrueByWith config buildProperty actual
        |> assertPassed
...
type Check =
    static member result
        ...
        : CheckResult<'Args, 'Actual, 'Expected> =
        match config, arbitrary with
        | Some resolvedConfig, Some resolvedArbitrary ->
            Check.checkUsingWith resolvedConfig resolvedArbitrary expectation reference actual
        | Some resolvedConfig, None ->
            Check.checkWith resolvedConfig expectation reference actual
        | None, Some resolvedArbitrary ->
            Check.checkUsing resolvedArbitrary expectation reference actual
        | None, None ->
            Check.check expectation reference actual
    ...
    static member should
        ...
        : unit =
        Check.result(expectation, reference, actual, ?config = config, ?arbitrary = arbitrary)
        |> Check.assertPassed
    ...
    static member resultBy
        ...
        match config with
        | Some resolvedConfig ->
            Check.checkByWith resolvedConfig buildProperty expectation reference actual
        | None ->
            Check.checkBy buildProperty expectation reference actual
    ...
    static member shouldBy
        ...
        : unit =
        Check.resultBy(buildProperty, expectation, reference, actual, ?config = config)
        |> Check.assertPassed
