...
let rec add (g: IFunction, h: IFunction): IFunction =
    { new IFunction with
        member self.ToString (): string = $"(%s{g.ToString ()} + %s{h.ToString ()})"
        member self.Apply (x: Nat): Nat = g.Apply x + h.Apply x
        member self.Derive (): IFunction = add (g.Derive (), h.Derive ())
    }
////endObjAdd)
...
let rec mul (g: IFunction, h: IFunction): IFunction =
    { new IFunction with
        member self.ToString (): string = $"(%s{g.ToString ()} * %s{h.ToString ()})"
        member self.Apply (x: Nat): Nat = g.Apply x * h.Apply x
        member self.Derive (): IFunction = add (mul (g.Derive (), h),  mul (g, h.Derive ()))
    }
////endObjMul)
...
let rec pow (g: IFunction, n: Nat): IFunction =
    { new IFunction with
        member self.ToString (): String = $"%s{g.ToString ()} ^ %s{show n}"
        member self.Apply (x: Nat): Nat = (g.Apply x) ** n
        member self.Derive (): IFunction = mul (mul (constant n, pow (g, n - 1N)), g.Derive ())
    }
////endObjPow)
...
let rec comp (g: IFunction, h: IFunction): IFunction =
    { new IFunction with
        member self.ToString (): String = $"(%s{g.ToString ()} o %s{h.ToString ()})"
        member self.Apply (x: Nat): Nat = g.Apply (h.Apply x)
        member self.Derive (): IFunction = mul (comp (g.Derive (), h), h.Derive ())
    }
////endObjComp)
