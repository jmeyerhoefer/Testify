...
type Lists1Tests () =
    ...
    member _.``a) plusOne Zufallstest`` (): unit =
        Assertify.Check <@ fun (xs: Nat list) -> Student.Lists1.plusOne xs = List.map (fun x -> x + 1N) xs @>
    ...
    member _.``b) filter Zufallstest`` (): unit =
        Assertify.Check <@ fun (p: Nat -> Bool) (xs: List<Nat>) -> Student.Lists1.filter p xs = List.filter p xs @>
    ...
    member _.``c) concat Zufallstest`` (): unit =
        Assertify.Check <@ fun (xs: List<Nat>) (ys: List<Nat>) -> Student.Lists1.concat xs ys = xs @ ys @>
    ...
    member _.``d) mirror Zufallstest`` (): unit =
        Assertify.Check <@ fun (xs: List<Nat>) -> Student.Lists1.mirror xs = List.rev xs @>
    ...
    member _.``e) sum Zufallstest`` (): unit =
        Assertify.Check <@ fun (xs: List<Nat>) -> Student.Lists1.sum xs = List.sum xs @>