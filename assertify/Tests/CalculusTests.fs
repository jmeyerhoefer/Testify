...
type CalculusTests () =
    ...
    member _.``OO: Apply Beispiele`` (): unit =
        Assertify.Check <@ fun (n: Nat) -> for func, _, f, _, _ in ex do (?) <@ func.Apply n = f n @> @>

    [<TestMethod; Timeout(10000)>]
    member _.``OO: Apply Zufall`` (): unit =
        Assertify.Check <@ fun (fct: Function) (n: Nat) -> (toOO fct).Apply n = testApply fct n @>
    ...
    member _.``Funktional: Apply Zufall`` (): unit =
        Assertify.Check <@ fun (fct: Function) (n: Nat) -> Student.Calculus.apply fct n = testApply fct n @>
