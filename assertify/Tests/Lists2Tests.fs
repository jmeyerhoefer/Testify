...
type Lists2Tests () =
    ...
    member _.``a) minAndMax Zufallstest2`` (): unit =
        ...
        Assertify.Check <@ fun (xs: List<Nat>) -> Student.Lists2.minAndMax xs = solution xs @>
    ...
    member _.``b) map Zufallstest`` (): unit =
        Assertify.Check <@ fun (p: Nat -> Bool) (xs: List<Nat>) -> Student.Lists2.map p xs = List.map p xs @>
    ...
    member _.``c) duplicate Zufallstest`` (): unit =
        Assertify.Check <@ fun (xs: List<Nat>) -> Student.Lists2.duplicate xs = List.foldBack (fun x acc -> x :: x :: acc) xs [] @>
    ...
    member _.``d) collect Zufallstest``() : unit =
        ...
        //             (fun x -> [ x * 2N; x * 3N ])
        //         ]
        // Assertify.Check <@ fun (xs: List<Nat>) -> FsCheck.FSharp.Prop.forAll fG (fun f -> Student.Lists2.collect f xs = List.collect f xs) @>
        ...
        Assertify.Check <@ fun (f: Nat -> Nat list) (xs: List<Nat>) -> Student.Lists2.collect f xs = List.collect f xs @>
    ...
    member _.``e) intersperse Zufallstest`` (): unit =
        Assertify.Check <@ fun (xs: List<Nat>) (x: Nat) -> Student.Lists2.intersperse x xs = intersperse x xs @>
    ...
    member _.``f) runs Zufallstest`` (): unit =
        Assertify.Check <@ fun (xs: List<Nat>) -> Student.Lists2.runs xs = runs xs @>